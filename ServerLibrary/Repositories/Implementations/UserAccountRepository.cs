...
namespace ServerLibrary.Repositories.Implementations
{
    public class UserAccountRepository(IOptions<JwtSection> config, AppDbContext appDbContext) : IUserAccount
    {
        public async Task<GeneralResponse> CreateAsync(Register user)
        {
            ...
            {
                var createAdmminRole = await AddToDatabase(new SystemRole() { Name = Constants.Admin });
                await AddToDatabase(new UserRole() { RoleId = createAdmminRole.Id, UserId = applicationUser.Id });
        ...
        public async Task<LoginResponse> SignInAsync(Login user)
        {
            ...
            var getUserRole = await FindUserRole(applicationUser.Id); //위 라인삭제 후 이렇게 수정
            if (getUserRole is null) return new LoginResponse(false, "user role not found");

            //var getRoleName = await appDbContext.SystemRoles.FirstOrDefaultAsync( _ => _.Id == getUserRole.RoleId);
            var getRoleName = await FindRoleName(getUserRole.RoleId);  //위 라인삭제 후 이렇게 수정
            if (getUserRole is null) return new LoginResponse(false, "user role not found");
        ...
        public async Task<LoginResponse> RefreshTokenAsync(RefreshToken token)
        {
            ...
            if (updateRefreshToken is null) return new LoginResponse(false, "사용자가 로그인하지 않았기 때문에 새로 고침 토큰을 생성할 수 없습니다.(Refresh token could not be generated because user has not signed in)");
